...
function normalizeCategory(raw) {
  ...
  if (containsAny(["치안", "범죄", "절도", "도둑", "폭행", "흉기", "경찰", "신고", "위협", "치", "싸움", "다툼", "취객", "주먹"])) {
    return "치안-범죄위험";
  }

  if (containsAny(["자연", "재난", "환경", "폭우", "침수", "홍수", "태풍", "지진", "미세먼지", "하수구", "역류", "자", "산사태", "흙", "토사", "무너", "빗물", "물", "비"])) {
    return "자연재난-환경";
  }

  if (containsAny(["위생", "보건", "쓰레기", "폐기물", "악취", "벌레", "쥐", "식중독", "곰팡이", "위", "얼룩", "습기", "화장실", "끈적"])) {
    return "위생-보건";
  }

  if (containsAny(["스팸", "spam", "세일", "광고", "홍보", "특가", "링크", "클릭", "스", "수익", "단톡", "카톡", "오픈채팅", "방"])) {
    return "스팸";
  }
...
function classifyByKeywords(title, body) {
  ...
  if (has(["치안", "범죄", "절도", "도둑", "폭행", "흉기", "경찰", "신고", "위협", "치", "싸움", "다툼", "취객", "주먹"])) {
    return "치안-범죄위험";
  }
  if (
    has([
